...
const exporter = async (
    ...
    const dataWithRelation = records.map((record) => ({
        ...
        communities: record.communities.map(
            (n: number) => listCommunities[n].name,
        ),
        institutes: record.institutes.map(
            (n: number) => listInstitutes[n].name,
        ),
        sections_cn: record.sections_cn.map(
            (n: number) => listSections[n].name,
        ),
    }));
...
const UnitsList = () => (
    ...
    >
        <Datagrid>
            ...
            <BooleanField
                source="active"
                label="resources.units.fields.active"
            />
            <EditButton />
            <DeleteWithConfirmButton />
        </Datagrid>
